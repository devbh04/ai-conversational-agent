...
def get_ist_time_context() -> str:
    ...
    return (
        ...
        "Always use ISO dates when calling book_appointment. Appointments in IST (+05:30).]"
    )
...
class DoctorTools(llm.ToolContext):
    ...
    @llm.function_tool(description="Transfer this call to Dr. Nehra or clinic staff. Use if: caller asks for doctor, is in emergency, or query is outside scope.")
    ...
    @llm.function_tool(description="End the call. Use ONLY when caller says bye/goodbye or after appointment is fully confirmed.")
    ...
    async def check_availability(
        ...
        try:
            resp = await _backend.get("/api/doc/availability", params={"date": date})
            data = resp.json()
            ...
            return f"Available slots on {date}: {data.get('formatted', '')}."
        except Exception as e:
            logger.error(f"[TOOL] check_availability failed: {e}")
            return "I'm having trouble checking the schedule right now."

    # ── Tool: Book Appointment (sync — waits for Cal.com confirmation) ─────
    @llm.function_tool(description="Book a dental appointment. Call this ONCE after you have name, phone, preferred time, and dental concern.")
    async def book_appointment(
        self,
        start_time:    Annotated[str, "ISO 8601 datetime for the appointment e.g. '2026-03-01T10:00:00+05:30'"],
        caller_name:   Annotated[str, "Full name of the patient"],
        caller_phone:  Annotated[str, "Phone number of the patient"],
        dental_concern:Annotated[str, "Brief description of dental issue: e.g. tooth pain, cleaning, checkup"],
        notes:         Annotated[str, "Any extra notes or requests"] = "",
    ) -> str:
        logger.info(f"[TOOL] book_appointment: {caller_name} at {start_time}")
        try:
            resp = await _backend.post("/api/doc/book-appointment", json={
            ...
            if result.get("success"):
                ...
                self.caller_name = caller_name
                self.dental_concern = dental_concern
                return f"Appointment confirmed for {caller_name} at {start_time}. WhatsApp confirmation will be sent."
            else:
                return f"Sorry, that slot is no longer available. {result.get('message', 'Please try another time.')}"
        except Exception as e:
            logger.error(f"[TOOL] book_appointment failed: {e}")
            return "I had trouble booking the appointment. Please try again."

    # ── Tool: Business Hours (sync — agent needs hours data) ──────────────
    @llm.function_tool(description="Check if Dr. Nehra's clinic is currently open and what the operating hours are.")
    async def get_business_hours(self) -> str:
        try:
            resp = await _backend.get("/api/doc/business-hours")
            data = resp.json()
            return data.get("message", "Unable to check clinic hours right now.")
        except Exception as e:
            logger.error(f"[TOOL] get_business_hours failed: {e}")
            return "Unable to check clinic hours right now."
...
async def entrypoint(ctx: JobContext):
    ...
    # ── Pre-fetch today + tomorrow slots (speed) ──────────────────────────
    ist = pytz.timezone("Asia/Kolkata")
    today_str = datetime.now(ist).strftime("%Y-%m-%d")
    tomorrow_str = (datetime.now(ist) + timedelta(days=1)).strftime("%Y-%m-%d")
    slot_task_today = asyncio.create_task(
        _backend.get("/api/doc/availability", params={"date": today_str})
    )
    slot_task_tomorrow = asyncio.create_task(
        _backend.get("/api/doc/availability", params={"date": tomorrow_str})
    )
    ...
    # ── Instantiate tools ─────────────────────────────────────────────────
    agent_tools = DoctorTools(caller_phone=caller_phone, caller_name=caller_name)
    ...
    async def unified_shutdown_hook(shutdown_ctx: JobContext):
        ...
        try:
            asyncio.create_task(_backend.post("/api/internal/call-completed", json={
                ...
                "dental_concern":  agent_tools.dental_concern,
